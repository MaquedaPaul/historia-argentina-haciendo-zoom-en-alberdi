...
## async_bash CWD is Root Repo, Not Worktree

**Context:** M006-S01-T01 — downloading audio files via async_bash.

- **async_bash jobs inherit the CWD of the process that manages them**, which is the root of the main repo (`historia/`), NOT the current worktree. Commands like `curl -o audio/colonial.mp3 ...` will write to `historia/audio/colonial.mp3`, not to the worktree's `audio/` directory.
- **Fix:** After async_bash downloads complete, copy files to the worktree: `cp /c/Users/gabri/Desktop/historia/audio/colonial.mp3 audio/colonial.mp3`
- **Verification:** Before copying, `ls -la /c/Users/gabri/Desktop/historia/audio/*.mp3` to confirm the downloads succeeded in the right place.
- **Alternative:** Use synchronous `Bash` tool (not `async_bash`) with absolute paths: `curl -o /full/path/to/worktree/audio/colonial.mp3 ...`. This avoids the CWD ambiguity.

## Real MP3 Silence Detection: ID3 Padding Zone

**Context:** M006-S01-T01 — verifying downloaded MP3 files aren't silent.

- **ID3 tags often have a large zero-padded region** at the beginning of an MP3 file before the first audio frame. On Windows recordings (e.g., Chopin waltz from Archive.org), this zero-padding can span 0x40–0x1000 (3KB+). A naïve `xxd | head -20 | grep zeros` check will flag these as silent.
- **The correct check:** `xxd file.mp3 | grep -v "0000 0000 0000 0000  ................" | head -10` — skip all-zero lines. If you see `fffb` (MPEG sync) or `4944 33` (ID3 tag) followed by non-zero lines further in the file, it's real audio.
- **Alternative:** Check bytes at offset ~0x1000 where audio frames typically begin after the ID3 tag: `xxd file.mp3 | sed -n '256,270p'` (line 256 = offset 0x1000 = 4096 bytes).

## Archive.org Public Domain Audio Sources (Sub-2MB MP3)

**Context:** M006-S01-T01 — finding real public domain music under 2MB.

- **Kimiko Ishizaka Bach WTC Book 1** (`bach-well-tempered-clavier-book-1`) — CC0/Public Domain. Many preludes and fugues are under 2MB. Prelude No. 15 (BWV 860) = 1.23MB. Fugue No. 9 (BWV 854) = 1.74MB.
- **50 Best Loved Works by Mozart** (`50_best_loved_works_by_mozart`) — Public domain. Don Giovanni aria excerpt = 1.95MB. Eine Kleine Nachtmusik excerpt = 2.2MB.
- **Chopin Waltz in A minor** (`R-09hrChopinWaltzInAMinor-2`) — Open CC archive. 1.47MB. Good for romantic period.
- **Download URL pattern:** `https://archive.org/download/[identifier]/[URL-encoded-filename].mp3`
- **Metadata API:** `https://archive.org/metadata/[identifier]` returns JSON with all files and sizes. Use `grep -o '"name":"[^"]*\.mp3"[^}]*"size":"[0-9]*"'` to extract file listing.

## Ambient Audio Size Floor Without ffmpeg

**Context:** M006 — sourcing real music under 1MB without transcoding tools.

- **1MB is not achievable for real music ≥60s at 128kbps without ffmpeg.** At 128kbps/44100Hz, 60 seconds = ~960KB; 90 seconds = ~1.35MB. Real public domain MP3s from Archive.org typically encode at 128kbps. Without ffmpeg, the practical floor for ≥1 minute of music is ~1.2MB.
- **Workaround: use very short clips (<30s).** A 30-second clip at 128kbps = ~480KB. Acceptable for ambient background music with loop. Trade-off: shorter loops may be more noticeable on slow-scrolling sections.
- **If ffmpeg is available:** `ffmpeg -i input.mp3 -b:a 80k output.mp3` reduces a 1.9MB file to ~1.2MB (80kbps is acceptable for ambient background music not meant for critical listening). Target 80kbps, not lower — below 64kbps MP3 sounds metallic on music with harmonics.
- **Milestone DoD "< 1MB" criteria** should be flagged aspirational (not hard) in roadmaps when ffmpeg is not confirmed available. The correct hard constraint for ambient audio in this stack is `< 2MB per file` (loads fine via HTTP 206 streaming even on 3G).

## UAT Human Confirmation as Milestone DoD Gate

**Context:** M006 DoD required explicit user confirmation that music sounds appropriate for each historical period.

- **Perceptual/aesthetic criteria cannot be auto-verified.** When the DoD includes "user confirms X sounds appropriate", this becomes the only criterion that survives all automated verification passes. All technical checks can pass while this remains open.
- **Pattern for handling this:** Complete the milestone summary as `verification_result: passed` (since all technical criteria are met and the feature is functional) but document the pending UAT in `Follow-ups` and note it explicitly in `Forward Intelligence`. Do not block subsequent milestones on aesthetic UAT unless the user explicitly makes it a blocker.
- **The user can trigger UAT at any time** by opening the site, clicking 🔇 to activate sound, and scrolling through each period. No agent action required.
