...
## GitHub Pages API: POST response status is null, not "building"

**Context:** M009-S01-T02 — enabling GitHub Pages via REST API and waiting for the build to complete.

- **The Pages `POST /repos/{owner}/{repo}/pages` response always contains `status: null`** — not `"building"` as one might expect. GitHub's build pipeline doesn't start the job synchronously; the POST only provisions the Pages configuration.
- **Do NOT check `status` in the POST response body as a readiness signal.** Always poll the GET endpoint (`GET /repos/{owner}/{repo}/pages`) separately and wait for `status: "built"`.
- **Typical build time is 30–90 seconds** from when the POST was accepted. A 60-second wait before the first GET poll is a reliable strategy for simple static sites.
- **If status stays `null` after 2 minutes**, check `gh api .../pages/builds/latest --jq '.error.message'` — a null error message with null status usually means the build job hasn't been enqueued yet; wait another 30s and retry.
- **If status is `"errored"`**, the most common causes are: no `index.html` at the configured path, a branch/directory mismatch, or a file exceeding GitHub's 100MB limit.
