...
## Stagger Delay Update When Inserting Cards Into an Existing events-grid

**Context:** M010-S02-T02 — inserting 11 new cards between SP1-1 and SP1-2 in #rev-1800-1820.

- **When N cards are inserted into an existing events-grid, all subsequent cards' `--reveal-delay` values must be updated** to maintain coherent stagger. The delay sequence is: existing last card + N×80ms for the new cards, then the first existing subsequent card gets +N×80ms, and so on.
- **Formula:** `new_delay_for_subsequent_card = original_delay + (N_inserted × 80ms)`. Example: inserting 11 cards means SP1-2 goes from 80ms → 960ms (80ms + 11×80ms).
- **Grep the range to confirm:** `grep -A3 "<!-- SP1-[2-5]:" index.html | grep "reveal-delay"` before and after insertion to verify the update. Missing this step produces a visual overlap where multiple cards appear simultaneously at the same delay as the first new card.
- **The `--reveal-delay: 0ms` on SP1-1 (or the first card) does not need updating** — it's the stagger anchor, not a relative value.

## Resumen de certeza Section: Epistemic Closure Pattern for Content Drafts

**Context:** M010-S01-T02 — closing a content draft with an explicit inventory of pending verification items.

- **Add a `## Resumen de certeza` section at the end of every content draft.** This section lists: (a) total card count, (b) certeza distribution (X hecho, Y debatido, Z opinión), and (c) an explicit inventory of all pending `[VERIFICAR]` flags organized by category (image URLs, data disputes, term verification). This lets the next-slice executor scan the entire epistemic state in <10 seconds without reading every card.
- **Failure-state check for drafts:** `(## sections) - (### Fuentes sections) = 1` (the Resumen section). If the difference is >1, a card is missing its Fuentes section. This arithmetic check catches silent gaps better than a full grep sweep.
- **Three levels of uncertainty markers:** `[VERIFICAR]` (claim uncertain, verify before use), `[VERIFICAR ATRIBUCIÓN]` (direct quote not verified against original text — must not be used as a blockquote), `[FUENTE PENDIENTE]` (image URL not verified via API — will produce broken `<img>` if used directly in HTML). Each level carries a different action requirement for the downstream slice.
- **Wikimedia image URL verification is always deferred to the HTML integration slice** when web_search quota is exhausted or curl/Python unavailable. Flag with `[FUENTE PENDIENTE]` in the draft. The integration slice uses the Wikimedia API protocol (KNOWLEDGE.md: Wikimedia Commons Image Sourcing) before writing any `<img src>` attribute.
- **Content draft Resumen counts can be wrong — always verify from card content, not the summary.** M010-S01 had an off-by-one: the Resumen wrote "hecho: 5" but listed 6 hecho items. When the integration slice's verification fails (e.g., `hecho: 7` when plan expects `6`), check whether the draft Resumen or the individual card classifications are the source of the discrepancy. Card content is always the source of truth.

## data-certeza="debatido" for Historiographic Dispute Cards

**Context:** M010-S02 — 5 cards covering contested historical events (vote count range, group identity, manipulation mechanism, post-25 first actions, popular vs. elite debate).

- **Use `data-certeza="debatido"` (not `"opinión"`) when the card covers a historiographically disputed fact** — i.e., where scholars disagree but no single prócer quote is being attributed. The `card-opinion` CSS class works for both `debatido` and `opinión` values; the difference is semantic, not visual.
- **`data-certeza="opinión"` is reserved for cards with a direct attributed quote** from a historical figure (prócer, leader, intellectual), where the card presents that person's viewpoint. A historiographic debate card with multiple scholarly positions is `debatido`, not `opinión`.
- **Both values use `class="event-card card-opinion"`** — the CSS class is shared, the certeza distinction is in the data attribute. Verification queries must account for both: `[data-certeza="opinion"], [data-certeza="opinión"], [data-certeza="debatido"]`.

## Omitting card-image When No Appropriate PD Image Exists

**Context:** M010-S02-T01 — Temática 4 (debate historiográfico sobre el 25 de mayo) had no public domain image available. Book covers require fair use; historian portraits would mislead readers into associating a visual with a concept.

- **Omit the `<div class="card-image">` entirely** when no appropriate public domain image exists for the card's subject. Do not force a marginally relevant image — it misleads readers about the card's content.
- **`initImageFallbacks()` in `app.js` only fires for `.card-image img` elements** — it does not throw errors or produce fallbacks for missing `.card-image` containers. A card without the div is structurally safe.
- **The `card-nota-historiografica` paragraph provides sufficient visual differentiation** for debate-type cards without an image. The typographic distinction (italic, indented, or styled paragraph) signals the card's special nature.
- **This pattern is distinct from image sourcing failure** — it's a deliberate editorial decision, not a fallback. Document the decision explicitly in the slice summary and image manifest so future agents don't "fix" the missing div.
